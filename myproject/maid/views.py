...
def maid_profile(request, pk):
	...
	if request.method == 'POST':
		...
		#reportpdf = write_pdf(template_name, {
			#'maids': maids,
		#})
		import pdfkit
		file_pdf = pdfkit.from_url('http://reliablemaid.com.sg/attach_maid_profile/'+str(maids.pk), str(maids.name) +'.pdf')
		pdf = open(str(maids.name) +'.pdf')

		file_nmae = str(maids.name) +'.pdf'
		...
		msg.attach(file_nmae, pdf.read(), "application/pdf")
		msg.send()
...
def attach_maid_profile(request, pk):
	"""
	Attach Maid Profile Page
	"""
	try:
		maids = Maid.objects.get(pk=pk)
	except Maid.DoesNotExist:
		maids = None
	return render_to_response('attach_maid_profile.html',{'maids':maids}, context_instance=RequestContext(request))
